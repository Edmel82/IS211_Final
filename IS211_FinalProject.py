...
def search():
    # This will if a user is logging in
    if 'user_id' not in session:
        return redirect('/')
    ...
    if request.method == 'POST':
        ...
        if 'items' in data:
            ...
            # Here we'll find a book in that's added to the search.
            for item in data['items']:
                ...
                # we won't include summary information of a book
                if description and search_query.lower() not in description.lower():
                    # we should store the book in the database with the user ID
                    conn.execute(
